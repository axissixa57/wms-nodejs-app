...
export function editProduct(req, res) {
    ...
    const productWeight = req.body.weight;
    const productCost = req.body.cost;
    const newProduct = { category: productCategory, name: productName, unit: productUnit, weight: productWeight, cost: productCost };
...
export function createConsignee(req, res) {
    ...
    const consignee = { _id: consigneeId, address: consigneeAddress, phone: consigneePhone };
...
export async function getWarehouseProducts(req, res) {
    ...
    const products = typeof req.query.idProduct == 'string' ? [req.query.idProduct] : req.query.idProduct;
    ...
    for (let i = 0; i < products.length; i++) {
        ...
        if (!result) {
            res.send({ message: `Код товара "${products[i]}" не числится на ${warehouseId} складе!` });
            isNotEmpty = false;
            break;
        }
    }
...
export function createWarehouse(req, res) {
    ...
    const warehouse = { _id: warehouseId, address: warehouseAddress, phone: warehousePhone };
...
export function addProductsToWarehouse(req, res) {
    ...
    sentProducts.forEach(async (product, i, arr) => {
        ...
        if (!wh) {
            ...
            console.log(`Код ${product.id} товара успешно добавлен на складе ${warehouseId}`);
            ...
            Warehouse.updateOne(
                ...
                function (err, warehouse) {
                    if (err) return console.log(err);
                    console.log(`Код ${product.id} товара успешно обновлён на складе ${warehouseId}`);
                }
            );
...
export async function minusQuantutyOfGoodsFromWarehouse(req, res) {
    if (!req.body) return res.sendStatus(400);
    ...
    for (let i = 0; i < sentProducts.length; i++) {
        ...
        if (!productFromWarehouse) {
            return res.send(`Код товара "${sentProducts[i].id}" не числится на этом складе!`);
        }
    }
    ...
    for (let i = 0; i < sentProducts.length; i++) {
        ...
        const result = parseInt(quantityFromWarehouse) - parseInt(sentProducts[i].quantity);
        ...
        Warehouse.updateOne(
            ...
            function (err, warehouse) {
                if (err) return console.log(err);
                console.log(`Код ${sentProducts[i].id} списан со склада ${warehouseId}!`);
            }
        );
...
export function addQuantutyOfGoodsToWarehouse(req, res) {
    ...
    async function updateQuntity(warehouseId, products) {
        for (let i = 0; i < products.length; i++) {
            const warehouseProducts = await Warehouse.findOne({ _id: warehouseId, "products.id_product": products[i].id }, { "products.$": 1 });
            const quantityFromWarehouse = warehouseProducts.products[0].quantity;
            const newQuantity = parseInt(quantityFromWarehouse) + parseInt(products[i].quantity);
...
export function returnQuantutyOfProductsToWarehouse(req, res) {
    ...
    for (let i = 0; i < products.length; i++) {
        Warehouse.findOne({ _id: warehouseId, "products.id_product": products[i].id }, { "products.$": 1 }, function (err, doc) {
            if (err) return console.log(err);
            const quantityFromWarehouse = doc.products[0].quantity;
            const result = parseInt(quantityFromWarehouse) + parseInt(products[i].quantity);
...
export async function getLastAddedTTN(req, res) {
    try {
        const result = await TTN.find().limit(1).sort({ $natural: -1 });
        res.send(result);
    } catch (err) {
        console.log(err);
    }
