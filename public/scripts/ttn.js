function countCost(input) {
  ...
  const price = parseFloat(
    input.parentNode.parentNode.parentNode.children[5].innerHTML
  );
...
function toColorizeSelectedPositionAndChooseValue(
  optionsAll,
  inputResult,
  btnChoose,
  someModalScrollable
...
function countTotalWeight() {
  ...
  for (let i = 0; i < trsMainTable.length; i++) {
    totalWeight +=
      parseFloat(trsMainTable[i].children[7].innerHTML) *
      parseInt(trsMainTable[i].children[4].children[0].children[0].value);
  }
...
$('#btnCreateDoc').click(async () => {
  ...
  const docStatus = $('#btnEnter').hasClass('btn-success')
    ? 'Заблокирован'
    : 'Разблокирован';
  const codeCar = $('#codeCar').val();

  const trsProductsTable = document.querySelectorAll(
  ...
  for (let i = 0; i < trsProductsTable.length; i++) {
    ...
    trsProductsTable[i].children[4].children[0].children[0].setAttribute(
      'data-quantity',
      trsProductsTable[i].children[4].children[0].children[0].value
    );
  }
...
$('#btnEnter').click(() => {
  ...
  for (let i = 0; i < trsProductsTable.length; i++) {
    ...
    warehouseProducts[i] = {
      ...
      quantity:
        parseInt(
          trsProductsTable[i].children[4].children[0].children[0].getAttribute(
            'data-quantity'
          )
        ) -
        parseInt(trsProductsTable[i].children[4].children[0].children[0].value)
    };
...
async function bootstrap(idWarehouse, idConsignee) {
  ...
  const cosmeticsProducts = await fetch('/api/products/cosmetics').then(res =>
    res.json()
  );
  const meatProducts = await fetch('/api/products/meat').then(res =>
    res.json()
  );
  ...
  if (warehouses) {
    ...
    const warehousesALL = document.querySelectorAll(
      '#all_counterAgents tbody tr'
    );
    const warehouseID = document.getElementById('warehouse_address');
    const btnWarehouses = document.getElementById('btnWarehouses');
    const counterAgentModalScrollable = document.getElementById(
      'counterAgentModalScrollable'
    );

    toColorizeSelectedPositionAndChooseValue(
      warehousesALL,
      warehouseID,
      btnWarehouses,
      counterAgentModalScrollable
  ...
  if (consignees) {
    ...
    const customerModalScrollable = document.getElementById(
      'customerModalScrollable'
    );
  ...
  if (marineProducts) {
    ...
    const marineDataTablebleTrs = document.querySelectorAll(
      '#marineDataTable tr'
    );
  ...
  if (idWarehouse && idConsignee) {
    ...
    if (parseInt(idConsignee) > 10000) {
      ...
      const consignee = await fetch(`/api/consignees/${id}`).then(res =>
        res.json()
      );
...
function GetMeatProducts() {
  $.ajax({
    ...
    success: function (products) {
      ...
      const meatDataTablebleTrs = document.querySelectorAll(
        '#meatDataTable tr'
      );
...
function fromOneWarehouseToAnother( 
  ...
  $.ajax({
    url: '/api/warehouse/add-quantity-from-one-to-another',
    contentType: 'application/json',
    method: 'PUT',
    data: JSON.stringify({
