...
templateRouter.get('/ttn/:id/ttn.pdf', async (req, res) => {
  ...
  const warehouseAddress = await Warehouse.find(
    { _id: ttn.id_warehouse },
    { address: 1 }
  );
  const consigneeAddress = await Consignee.find(
    { _id: ttn.id_consignee },
    { address: 1 }
  );
  ...
  ejs.renderFile(
    path.join(__dirname, '../views/templates/ttn/template.ejs'),
    obj,
    (err, html) => {
...
templateRouter.get('/shipping-list.pdf', async (req, res) => {
  const idsShipments =
    typeof req.query.id == 'object'
      ? req.query.id.map(item => parseInt(item))
  ...
  ejs.renderFile(
    path.join(__dirname, '../views/templates/shipments/template.ejs'),
    obj,
    (err, html) => {
      const options = { format: 'A4', orientation: 'landscape' };
      const fileName = path.join(
        __dirname,
        '../views/templates/shipments/file.pdf'
      );
      ...
      htmlPdf.create(html, options).toFile(fileName, err => {
        ...
        // res.end(html);
        res.sendFile(
          path.join(__dirname, '../views/templates/shipments/file.pdf')
        );
      });
    }
  );
